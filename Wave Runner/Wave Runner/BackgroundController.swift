...
class BackgroundController {
    ...
    func config(parent: SKNode) -> Void {
        ...
        background2.position = CGPoint(x: background1.position.x + background1.size.width - 1, y: background2.position.y)
    ...
    func run(parent: SKNode) -> Void {
        parent.enumerateChildNodes(withName: "background", using: {
            ...
            if background.position.x <  -background.size.width {
                background.position = CGPoint(x: background.position.x + background.size.width * 2 - 2, y: background.position.y)
            }
        })
    }
}
